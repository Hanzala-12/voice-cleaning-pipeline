...
class SpectralRestoration:
    ...
    def compute_spectral_envelope(self, audio: np.ndarray, n_cepstral: int = 20) -> np.ndarray:
        """
        ...
        log_magnitude_full = np.concatenate([
            log_magnitude,
            log_magnitude[-2:0:-1]
        ])
    ...
    def synthesize_harmonics(self, 
        ...
        for k in range(2, n_harmonics + 2):  # Start from 2nd harmonic
            ...
            if harmonic_freq < self.sample_rate / 2:
                ...
                # Synthesize sinusoid
                harmonic_signal = amplitude * np.sin(2 * np.pi * harmonic_freq * t + phase)
    ...
    def apply_perceptual_weighting(self, 
                                   spectrum: np.ndarray,
        ...
        # Compute A-weighting
        numerator = c1**2 * f**4
        denominator = ((f**2 + c2**2) * 
                      np.sqrt((f**2 + c3**2) * (f**2 + c4**2)) * 
                      (f**2 + c1**2))
    ...
    def restore_high_frequency(self, 
                              original: np.ndarray,
                              processed: np.ndarray,
                              restoration_strength: float = 0.3) -> np.ndarray:
        """
        ...
        """
        logger.info(f"Starting spectral restoration (strength={restoration_strength:.2f})...")
        ...
        for i in range(0, len(processed) - frame_length, hop_length):
            ...
            if pitch > 0 and 80 <= pitch <= 400:  # Valid speech pitch
                ...
                restored_audio[i:i + frame_length] += (restored_frame - frame) * window
    ...
    def adaptive_restoration(self,
        ...
        logger.info(f"HF energy retained: {energy_retention*100:.1f}%, "
