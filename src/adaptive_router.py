...
class AdaptiveRouter:
    ...
    def lightweight_filter(self, audio: np.ndarray) -> np.ndarray:
        ...
        noise_frames = []
        for i in range(0, min(10 * frame_length, len(audio) - frame_length), frame_length):
    ...
    def moderate_filter(self, audio: np.ndarray) -> np.ndarray:
        ...
        for i in range(0, len(audio) - frame_length, hop_length):
            ...
            wiener_gain = np.maximum(
                1 - (noise_power / (power_spectrum + 1e-10)),
                h_min
            )
            
            # Apply gain
            enhanced_spectrum = np.abs(frame_spectrum) * wiener_gain * np.exp(1j * phase)
            enhanced_frame = np.fft.irfft(enhanced_spectrum, n=frame_length)
    ...
    def route_processing(self,
                        audio: np.ndarray,
                        noise_profile: Dict,
                        heavy_processor: Callable = None) -> Tuple[np.ndarray, str]:
        """
        ...
        elif (snr < self.thresholds['snr_heavy'] or 
              noise_var > self.thresholds['noise_var_threshold'] or
              spectral_flatness > self.thresholds['spectral_flatness_threshold']):
            decision = 'heavy'
            logger.info(f"✓ ROUTE: Heavy (SNR={snr:.1f}dB, very noisy)")
        
        # Rule 3: Moderate noise → moderate processing
        elif snr >= self.thresholds['snr_heavy'] and snr <= self.thresholds['snr_moderate']:
            decision = 'moderate'
    ...
    def adaptive_dual_path(self,
                          audio: np.ndarray,
                          noise_profile: Dict,
                          heavy_processor: Callable = None,
                          blend_factor: float = None) -> np.ndarray:
        """
